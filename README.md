...
> นี่คือ **Phase 1 (MVP) + แจ้งเตือนกลับ LINE**: ดักงานด้วย keyword `/task`, Kanban 4 คอลัมน์, drag & drop, รับงาน, realtime
> และ **push แจ้งความคืบหน้ากลับเข้ากลุ่ม LINE** ทุกครั้งที่สถานะเปลี่ยน / มีคนรับงาน
> ส่วน AI auto-detect / รายงาน คือ Phase ถัดไป (ดู `00-system-design.md`)
...
| `docker-compose.yml` | PostgreSQL สำหรับ dev + backend container (profile `full`) |
| `backend/Dockerfile` | Build image ของ backend (รัน migration อัตโนมัติตอน start) |
...
4. แท็บ **Messaging API** → **Webhook settings**:
   - เปิด tunnel ก่อน: `ngrok http 3000` → ได้ URL เช่น `https://xxxx.ngrok.io`
   - ตั้ง **Webhook URL** = `https://xxxx.ngrok.io/webhook` → กด **Verify** (ต้องขึ้น Success)
   - เปิด **Use webhook** = ON
5. ปิด auto-reply ของ OA (ไม่งั้นบอตจะตอบข้อความอัตโนมัติแทรกตลอด):
   ไปที่ **LINE Official Account Manager** (https://manager.line.biz) → Settings → Response settings
   - **Chat** = OFF, **Auto-response** = OFF, **Webhooks** = ON, **Greeting message** = OFF
6. ที่ OA Manager เดียวกัน → Settings → Account settings →
   เปิด **Allow bot to join group chats** (ค่าเริ่มต้นปิดอยู่ — ถ้าไม่เปิด เชิญบอตเข้ากลุ่มไม่ได้)

### ทดลองใช้

7. เชิญ bot เข้ากลุ่ม → บอตจะทักทายพร้อมสอนวิธีใช้อัตโนมัติ
8. พิมพ์ในกลุ่ม:
...
### แจ้งเตือนกลับเข้ากลุ่ม (ใหม่ ✨)

ทุกครั้งที่มีความเคลื่อนไหวบนบอร์ด บอตจะ push ข้อความเข้ากลุ่มที่งานนั้นมาจาก:

- ลากการ์ดเปลี่ยนคอลัมน์ → `งาน: แก้ปุ่ม login หน้าแรก` + `สถานะ → 🔧 In Process (กำลังทำ)`
- กดรับงาน → `🙋 สมชาย รับงาน "แก้ปุ่ม login หน้าแรก" แล้ว`
- สถานะ: 📥 Todo (รอรับงาน) · 🔧 In Process (กำลังทำ) · 🧪 Test (กำลังเทส) · ✅ Done (เสร็จแล้ว)

> push ใช้ quota ข้อความของ OA (แพ็กเกจฟรีมีลิมิตต่อเดือน) — ถ้า push ล้มเหลว ระบบจะ log warning แต่บอร์ดยังทำงานปกติ

---

## รันทั้งชุดด้วย Docker (db + backend)

```bash
cp backend/.env.example backend/.env   # แล้วใส่ค่า LINE channel ให้ครบ
docker compose --profile full up -d --build
```

- backend จะรอ db พร้อม (healthcheck) → รัน migration → เปิดที่ `:3000`
- `docker compose up -d` เฉย ๆ (ไม่ใส่ profile) = ได้แค่ Postgres เหมือนเดิม สำหรับ dev ที่รัน backend ด้วย `npm run start:dev`
...
| PATCH | `/tasks/:id/status` | เปลี่ยนสถานะ (ตอนลากการ์ด) body: `{ "status": "in_process" }` — push แจ้งเข้ากลุ่ม LINE ด้วย |
| POST | `/tasks/:id/assign` | รับงาน body: `{ "userId": "...", "displayName": "..." }` — push แจ้งเข้ากลุ่ม LINE ด้วย |
...
- auth จริงบนบอร์ด (LINE Login) + production build ของ frontend
